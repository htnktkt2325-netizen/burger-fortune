...
function Stars({ count }) {
  ...
  return (
    <span>
      {[1,2,3,4,5].map(i => (
        <span key={i} style={{ color: i <= n ? "#FFB800" : "#333", fontSize: 15 }}>★</span>
      ))}
    </span>
  );
}
...
export default function BurgerFortune() {
  ...
  return (
    <div style={{
      ...
      color: "#fff", padding: "14px 13px 32px", boxSizing: "border-box",
    }}>
      <style>{`
        * { box-sizing: border-box; }
        @keyframes spin { to { transform: rotate(360deg); } }
        @keyframes pop  { 0%{transform:scale(0.93);opacity:0} 100%{transform:scale(1);opacity:1} }
        .card { animation: pop 0.3s ease forwards; }
      ...
      <div style={{ textAlign:"center", marginBottom:14 }}>
        ...
        <h1 style={{
          margin:"2px 0 1px", fontSize:20, fontWeight:"bold", letterSpacing:3,
      ...
      <div style={{
        ...
        borderRadius:13, padding:"14px", marginBottom:12,
      }}>
        <p style={{ color:"#AA7744", fontSize:11, textAlign:"center", margin:"0 0 9px" }}>
          🔮 誕生日を入力してください
        </p>
        <input
          type="date"
          ...
          style={{
            width:"100%", padding:"10px 11px", borderRadius:8,
            border:"1.5px solid rgba(255,184,0,0.3)",
            background:"rgba(0,0,0,0.4)", color:"#FFD580",
            fontSize:14, outline:"none", marginBottom:9,
          }}
        />
        {error && (
          <div style={{
            color:"#FF9090", fontSize:12, marginBottom:8,
            background:"rgba(255,0,0,0.08)", borderRadius:6, padding:"7px 10px", lineHeight:1.5,
          }}>⚠️ {error}</div>
        )}
        <button className="btn" onClick={getFortune} disabled={loading} style={{
          width:"100%", padding:"12px", borderRadius:9, border:"none",
          background: loading ? "#222" : "linear-gradient(135deg,#FF8C00,#FFB800)",
          color: loading ? "#555" : "#1a0a00",
          fontWeight:"bold", fontSize:14, cursor: loading ? "not-allowed" : "pointer",
          letterSpacing:1, transition:"opacity 0.15s",
        }}>
          {loading
            ? <><span style={{ display:"inline-block", animation:"spin 0.8s linear infinite" }}>🍔</span> 占い中…</>
            : "🍔  バーガーで占う！"}
        </button>
      </div>
      ...
      {fortune && !loading && (
        <div className="card">
          {/* Tabs */}
          <div style={{
            display:"flex", marginBottom:10, borderRadius:9, overflow:"hidden",
            ...
            {[["fortune","🔮 運勢"],["post","📋 投稿文"]].map(([k,lbl]) => (
              <button key={k} onClick={() => setTab(k)} style={{
                ...
                background: tab===k ? "rgba(255,184,0,0.18)" : "transparent",
                color: tab===k ? "#FFB800" : "#555", transition:"all 0.2s",
              }}>{lbl}</button>
            ))}
          </div>
          ...
          {tab === "fortune" && (
            <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
              <div style={{
                background:"linear-gradient(135deg,rgba(255,184,0,0.1),rgba(255,107,0,0.07))",
              ...
              <div style={{
                ...
                {[
                  ["💕 恋愛運", fortune.score.love,  fortune.love],
                  ["💼 仕事運", fortune.score.work,  fortune.work],
                  ["💰 金　運", fortune.score.money, fortune.money],
              ...
              <div style={{
                background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.07)",
                borderRadius:11, padding:"10px 13px", display:"flex", gap:10,
              }}>
                <div style={{ flex:1 }}>
                  <p style={{ margin:"0 0 2px", color:"#553311", fontSize:9, letterSpacing:1 }}>ラッキートッピング</p>
                  <p style={{ margin:0, color:"#FFB800", fontSize:12 }}>🥒 {fortune.lucky_topping}</p>
                </div>
                <div style={{ flex:1 }}>
                  <p style={{ margin:"0 0 2px", color:"#553311", fontSize:9, letterSpacing:1 }}>ラッキーカラー</p>
                  <p style={{ margin:0, color:"#FFB800", fontSize:12 }}>🎨 {fortune.lucky_color}</p>
                </div>
              </div>
          ...
          {tab === "post" && (
            <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
              <div style={{
                ...
                <pre style={{
                  margin:0, color:"#CCC", fontSize:11,
              ...
              <button className="btn" onClick={copy} style={{
                ...
                border:"1px solid rgba(255,184,0,0.25)",
                background: copied ? "rgba(50,200,100,0.12)" : "rgba(255,184,0,0.07)",
                color: copied ? "#50C864" : "#FFB800",
                fontSize:13, cursor:"pointer", fontWeight:"bold", transition:"all 0.3s",
              }}>
